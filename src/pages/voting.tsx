...
const Voting = () => {
    ...
    return (
        <section className="max-w-lg mx-auto block">
            ...
            {poll.title && <h1 className="text-xl font-medium block w-full bg-transparent pb-3 sm:text-2xl lg:text-3xl mb-10 text-white border-b border-gray-400">{poll.title}</h1>}
            {((Cookies.get('pollCreatedByCurrentUser') || poll.closed) || Cookies.get('voted')) ? poll.options.map((value: string, i: number) => {
